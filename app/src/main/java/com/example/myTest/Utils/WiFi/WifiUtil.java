package com.example.myTest.Utils.WiFi;
...
//    forgetWifi(SSID);
    ...
    //打开热点
    public void createWifiHot(String name, String password) {
        try {
    ...
    //   public enum WifiCipherType { NONE,WEP,WPA,EAP }
    //开启热点
    public void createWifiHot(String name, String password, String type) {
        WifiConfiguration config = new WifiConfiguration();
        if (mWifiManager.isWifiEnabled())
            mWifiManager.setWifiEnabled(false);//如果WiFi是开启的就关闭WiFi。
            ...
            case "WPA2":
                ...
                for (int i = 0; i < WifiConfiguration.KeyMgmt.strings.length; i++) {
                    if ("WPA2_PSK".equals(WifiConfiguration.KeyMgmt.strings[i])) {
                        config.allowedKeyManagement.set(i);//直接给它赋索引的值
